...
--  Track selling operations for recovery
local sellQueue = {}
local failedSells = {}
...
--  Enhanced selling with recovery capabilities and dialog suppression
function Scrappy.Recovery.StartSelling(items)
    ...
    --  Set up dialog auto-acceptance for soulbound warnings
    ...
    --  Convert items to sell queue entries
...
--  Process the sell queue with recovery
function Scrappy.Recovery.ProcessSellQueue()
    ...
    --  Validate the item still exists and is the same
    ...
    if not currentItem or currentItem.itemID ~= entry.itemID then
        --  Item moved or changed, skip it
        Scrappy.Recovery.ProcessSellQueue()
        return
    end
    
    --  Check if item is locked (being sold)
    if currentItem.isLocked then
        --  Item is locked, retry after delay
    ...
    if success then
        ...
        --  Sell failed, retry or mark as failed
        if entry.attempts <= MAX_SELL_RETRIES then
            C_Timer.After(SELL_RETRY_DELAY, function()
                table.insert(sellQueue, 1, entry) -- Re-add to front of queue
                Scrappy.Recovery.ProcessSellQueue()
...
--  Finalize selling process and report results
...
function Scrappy.Recovery.RetryFailedSells()
    ...
    --  Convert failed items back to regular items for selling
    local retryItems = {}
    for _, failed in ipairs(failedSells) do
        --  Re-validate the item still exists
...
--  Cancel ongoing selling operation
...
--  Emergency stop for when things go wrong
...
--  Set up automatic handling of soulbound confirmation dialogs
function Scrappy.Recovery.SetupDialogHandling()
    --  Only set up if auto-confirm is enabled
    if not ScrappyDB.autoConfirmSoulbound then return end
    
    --  Hook StaticPopup_Show to intercept soulbound confirmations
    if not Scrappy.Recovery.originalStaticPopupShow then
        ...
        StaticPopup_Show = function(which, text_arg1, text_arg2, data, ...)
            --  Handle different types of sell confirmations
            ...
            --  For all other popups, use normal behavior
            return Scrappy.Recovery.originalStaticPopupShow(which, text_arg1, text_arg2, data, ...)
        end
    end
end

--  Clean up dialog handling when selling is complete
function Scrappy.Recovery.CleanupDialogHandling()
    --  Restore original StaticPopup_Show if we hooked it
